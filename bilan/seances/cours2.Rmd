...
Dans les méthodes hiérarchiques, on construit différents niveau de
partition puis on choisit a posteriori selon des critères définis le
nombre de clusters à retenir.

En CAH, la méthode est la suivante :

-   (Construction d'un plan ou espace dans lequel les objets à
    classifier se trouvent)

-   Choix de la mesure de distance (euclidienne, ... )

-   Choix du critère aggrégatif entre les groupes

-   Mise en oeuvre de la méthode, calculs des distances entre les
    objets, à chaque étape, les objets les plus proches (plus faible
    distance les séparant) sont regroupés dans un groupe. Pour aggréger
    des groupes, on se base sur le critère d'agrégation.

-   Construction du dendogramme

-   Choix a posteriori de la variable latente, de classe.

Voici un schéma de ce processus. La distance est une distance
euclidienne et le critère d'aggrégation est un critère de distance
moyenne (distance des centres de gravité).

```{r, echo = F}
knitr::include_graphics('seances/images/CAH1.png')
```

```{r, echo = F}
knitr::include_graphics('seances/images/CAH2.png')
```
...
```{r, echo = F}
knitr::include_graphics('seances/images/saumon.png')
```
...
-   Un bloc « noir » : situé sous le bloc rouge, il présente les saumons
    ...
    individus) en lignes (ex : taux de sel, etc)
...
préférences de consommation. Ceci doit permettre de comprendre les
...
dta_saumon<-read.table("data/saumon.csv",
...
#### Etape 1 : PCA sur dta_saumon_R

```{r}
res.pca.1<-PCA(dta_saumon_R,graph = TRUE, ncp = 30)
# res.pca.1$eig
res.HCPC1 <- HCPC(res.pca.1, nb.clust = -1) # avec nb.clust = -1 : choix optimal du nombre de clusters 
...
# data frame individu actifs et de toutes les variables actives, illustratives, ainsi que de la variable de classe ("clust"=> la dernière variable)

# 2 - "desc.var"  
# description des classes (clusters)
res.HCPC1$desc.var$category # v test pour les clusters (test de surreprésentation de la modalité) => modalités caractérisant les groupes
 ### rappels 
 ## CLA/MOD : % d'individus qui prennent cette modalité pour cette variable et qui sont dans CE CLUSTER  
 ## MOD/CLA : % d'individus DE CE CLUST QUI ONT PRIS cette modalité pour cette variable 
 ## Global  : % d'individus en global qui prennent cette modalité pour cette variable
 ## p value : proba associé d'observer sous H0 une valeur de v test au moins aussi élevée que celle observée réellement
 ## v. test : valeur de la stat de test 
...
# 4 - "desc.ind"   
res.HCPC1$desc.ind$para # individus les plus proches des centres des classes
res.HCPC1$desc.ind$dist # individus les plus éloignés des centres des autres classes
```

`{catdes(res.HCPC1$data.clust, num.var = 31) # on cherche à comprendre les clusters}`

Avec la fonction "cat des" on réalise la description d'une variable
catégorielle (la variable de classe) par les variables quantitatives. On
peut en déduire les saumons les plus appréciés (avec des notes
d'appréciation hédonique significativement plus élevées que dans les
autres clusters) et ceux les moins appréciés par les individus de ce
clusters (resp., avec les notes en moyenne significativement plus
faibles que dans les autres clusters).

Ainsi, on en déduit que :

-   les individus du cluster 1 apprécient les saumons **16, 26, 18, 25
    et 28** (leur top 5), et n'aiment pas les saumons **10, 29, 17, 7 et
    23** (leur bottom 5).
-   les individus du cluster 2 apprécient les saumons **7, 4, 27, 17 et
    13** (leur top 5), et n'aiment pas les saumons **16, 12, 8, 30 et
    20** (leur bottom 5).
-   les individus du cluster 3 apprécient les saumons **20, 29, 10, 9 et
    23** (leur top 5), et n'aiment pas les saumons **26, 28, 27, 25 et
    11** (leur bottom 5).

On peut aussi savoir les produits qui séparent le plus les groupes entre
eux : il s'agit des saumons **16, 26, 10 et 29**.

#### Etape 2 : PCA sur dta_saumon_R + var illus B

```{r}
res.pca.2<-PCA(dta_saumon_RB [c(-1,-2)],
               quali.sup = c(31:98),
               graph = TRUE)

res.HCPC2 <- HCPC(res.pca.2, nb.clust = -1)

res.HCPC2$desc.var

```

#### Etape 3 : PCA sur dta_saumon_R + var illus B + ind illus N
...
```{r, message= F, warning=FALSE, fig.width=8 , fig.height= 6}
...
col_ind_sup <- c("#E41A1C", "#377EB8", "#E41A1C", "#E41A1C", 
...
col_quali_sup <- c(rep("red4",6), rep("blue4",3),rep("green4",3))

p1 <- combined %>%
  ggplot() +
  aes(x = Dim.1, y = Dim.2, color = factor(clust)) +
  ...
  # Points supplémentaires individus
  geom_point(data = ind_sup, 
             aes(x = Dim.1, y = Dim.2),
             color = col_ind_sup,
             size = 2, 
             shape = 16) +
  ...
  geom_text_repel(data = ind_sup, 
                  aes(label = row.names(ind_sup)),
                  color = col_ind_sup, 
                  size = 4) + 
  
  # Points supplémentaires qualitatifs
  geom_point(data = quali_sup, 
             aes(x = Dim.1, y = Dim.2),
             color = col_quali_sup,
             size = 2, 
             shape = 16) +
